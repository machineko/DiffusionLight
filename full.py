...
GUIDANCE_SCALE = 5.0
SEED = random.choice(
    (0, 37, 71, 125, 140, 196, 307, 434, 485, 575, 9021, 9166, 9560, 9814)
)
EV = "0,-2.5,-5"
DEVICE = "cuda"
MAX_NEGATIVE_EV = -5
...
def get_ball_location(image_data):
    if "boundary" in image_data:
        ...
        if x + r + half_dilate > IMG_W:
            x -= half_dilate
        if y + r + half_dilate > IMG_H:
            y -= half_dilate
...
def interpolate_embedding(pipe):
    ...
    prompt_embeds_normal, _, pooled_prompt_embeds_normal, _ = (
        pipe.pipeline.encode_prompt(prompt_normal)
    )
    prompt_embeds_dark, _, pooled_prompt_embeds_dark, _ = pipe.pipeline.encode_prompt(
        prompt_dark
    )
    ...
    for t in interpolants:
        int_prompt_embeds = prompt_embeds_normal + t * (
            prompt_embeds_dark - prompt_embeds_normal
        )
...
def process_image(ball_img):
    ...
    with torch.no_grad():
        ...
        env_map = torch.nn.functional.grid_sample(
            ball_image, grid, mode="bilinear", padding_mode="border", align_corners=True
        )
        env_map = env_map[0].permute(1, 2, 0).numpy()

    env_map_default = skimage.transform.resize(
        env_map, (BALL_SIZE, BALL_SIZE * 2), anti_aliasing=True
    )
